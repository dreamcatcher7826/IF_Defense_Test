...
import sys
#sys.path.append('../')
#sys.path.append('/home/drg/IF-Defense-main/baselines/')
sys.path.append('/public/home/xiaoyu_zhou/usr22/syq_inte22/IF-Defense-main/baselines/')

from config import BEST_WEIGHTS
from config import MAX_ADD_OBJECT_BATCH as BATCH_SIZE
...
from attack import CWAddObjects
from attack import CrossEntropyAdvLoss, LogitsAdvLoss
from attack import L2ChamferDist
...
if __name__ == "__main__":
    ...
    parser.add_argument('--obj_num_p', type=int, default=64, metavar='N',
                        help='Number of points in each cluster')
    parser.add_argument('--obj_root', type=str,
    ...
    dist_func = L2ChamferDist(num_add=args.num_add,
                              chamfer_method='adv2ori',
                              chamfer_weight=0.2)
    attacker = CWAddObjects(model, adv_func, dist_func,
                            object_pc=np.load(args.obj_root),
                            attack_lr=args.attack_lr,
                            init_weight=5., max_weight=40.,
                            binary_step=args.binary_step,
                            num_iter=args.num_iter,
                            num_add=args.num_add,
                            obj_num_p=args.obj_num_p,
                            scaling=0.3)
    ...
    # save results
    save_path = './attack/results/{}_{}/Object/{}-{}'.\
        format(args.dataset, args.num_points, args.num_add, args.obj_num_p)
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if args.adv_func == 'logits':
        args.adv_func = 'logits_kappa={}'.format(args.kappa)
    save_name = 'Object-{}-{}-success_{:.4f}-rank_{}.npz'.\
